...
function save_post_meta_fgc_quiz_setting($post_id) {
    $number_submit = (int) $_POST['number-submit']; // when add new
    if($number_submit) {
...
function register_meta_box_select_quiz() { // 1
        if(get_post_type() == 'exercise') 
            add_meta_box( 'list-quiz', 'Import quiz', 'print_box_select_quiz','exercise','advanced','high');
    }
...
function fgc_savePointAjax() {
    ...
    if(isset($_REQUEST['func']) && $_REQUEST['func']=='completedQuiz') {
        ...
        if(!$query || (count($query) < $number_submit)) {
            $wpdb->insert(FGC_QUIZ_TABLE_POINT, array('post_id' => $post_id, 'quiz_id' => $quiz_id, 'user_id' => $user_id,'point'=>$point));
        }
    }
}
